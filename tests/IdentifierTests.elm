...
{-| An unchecked way to construct a new identifier.
-}
newIdentifier : String -> String -> String -> Identifier
newIdentifier n c i =
    { namespace = n, collection = c, identifier = i }
    ...
    describe "toString"
        [ test "Can convert an Identifier to a String" <|
            \_ ->
                let
                    result =
                        Identifier.toString (newIdentifier "na" "co" "id")
                in
        ...
        , test "Resulting String is not hardcoded" <|
            \_ ->
                let
                    result =
                        Identifier.toString (newIdentifier "a" "b" "c")
                in
        ...
        , describe "Well-formed identifiers"
            [ test "are parsed correctly" <|
                \_ ->
                    let
                        expected =
                            Just (newIdentifier "m" "l" "h")

                        actual =
                            Identifier.parse "m:l:h"
                    in
            ...
            , test "are not parsed to a hardcoded value" <|
                \_ ->
                    let
                        expected =
                            Just (newIdentifier "z" "zz" "zzz")

                        actual =
                            Identifier.parse "z:zz:zzz"
                    in
                    Expect.equal expected actual
            ]
